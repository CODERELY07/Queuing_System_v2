...
MedQueue uses Laravel Reverb (a self-hosted WebSocket server) with Laravel Echo on the frontend. There's exactly one public broadcast channel — `queue` — and three events on it.
...
All three implement `ShouldBroadcastNow` (synchronous — no queue worker needed) and broadcast the same shape: `{id, name, status, service_id, queue_number}`.
...
`app/Events/*.php` — all three are near-identical; `QueueNextEvent`'s constructor also logs the payload via `Log::debug()` for local debugging.
...
The staff dashboard itself does **not** listen on any of these — it only refreshes after its own actions (see [staff-dashboard.md](staff-dashboard.md#how-the-dashboard-stays-in-sync)), so two staff accounts sharing one department's counter won't see each other's changes without taking an action themselves. In practice each department has exactly one staff account, so this hasn't been a problem.
